...
class ContentController extends AdminBase {
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            if($_POST['info']['address']=="") unset($_POST['info']['address']);
    ...
    {
        //时间范围搜索
        $start_uploadtime = I('get.starttime');
        $end_uploadtime = I('get.endtime');
        if (!empty($start_uploadtime)) {
            $where['create_time'] = array('EGT', strtotime($start_uploadtime));
            if ($end_uploadtime) {
                $where['create_time'] = array(array('EGT', strtotime($start_uploadtime)), array('ELT', strtotime($end_uploadtime)), 'AND');
            }
        }
        if($_GET['phone']){
        ...
        $page = $this->page($count, 20);
        $data = M('subscribe')->where($where)->limit($page->firstRow . ',' . $page->listRows)->order(array("create_time" => "DESC"))->select();
    ...
    public function delete_yuyue()
    {
        if (IS_POST) {
            $aid = $_POST['id'];
            foreach ($aid as $k => $v) {
                M('subscribe')->where(['id'=>(int)$v])->delete();
            }
            $status = true;
        } else {
            ...
            if (M('subscribe')->where(['id'=>(int)$aid])->delete()) {
                $status = true;
            } else {
                $status = false;
            }
